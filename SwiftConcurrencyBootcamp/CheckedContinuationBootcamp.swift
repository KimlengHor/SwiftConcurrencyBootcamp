...
class CheckedContinuationBootcampViewModel: ObservableObject {
    ...
    let networkMananger = CheckedContinuationBootcampDataManager()
    
    func getImage() async {
        guard let url = URL(string: "https://picsum.photos/300") else {return}
        
        do {
            let data = try await networkMananger.getData2(url: url)
    ...
    func getHeartImage() async {
        let image = await networkMananger.getHeartImageFromDatabase()
        self.image = image
    }
}
